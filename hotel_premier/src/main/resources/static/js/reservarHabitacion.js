...
//     const [fechaIng, horaIng] = fechaInicio.split(' ');
//     const [fechaEgr, horaEgr] = fechaFin.split(' ');
    
//     // Construir URL con parámetros
//     const params = new URLSearchParams({
//         habitacion: seleccionReserva.habitacionNumero,
//         tipo: seleccionReserva.tipoHabitacion, // Asegúrate de tener este dato
...
// Array para almacenar todas las reservas pendientes
let reservasPendientes = [];

// Objeto para la selección actual (temporal)
let seleccionReserva = {
    habitacionNumero: null,
    habitacionId: null,
    tipoHabitacion: null,
...
function reservar(boton, numero, id) {
    ...
    // Primer click: seleccionar habitación y fecha inicial
    if (!seleccionReserva.habitacionNumero) {
        // Obtener el tipo de habitación desde la fila
        const tipoHabitacion = obtenerTipoHabitacionDesdeBoton(boton);
        
        seleccionReserva.habitacionNumero = numero;
        seleccionReserva.habitacionId = id;
        seleccionReserva.tipoHabitacion = tipoHabitacion;
        seleccionReserva.primerFecha = fechaCelda;
        seleccionReserva.primeraSeleccion = boton;
    ...
    if (seleccionReserva.habitacionNumero === numero && !seleccionReserva.segundaFecha) {
        ...
        // Verificar disponibilidad y agregar a reservas pendientes
        verificarYAgregarReserva();
    }
}

function obtenerTipoHabitacionDesdeBoton(boton) {
    // Obtener la fila que contiene el botón
    const fila = boton.closest('tr');
    // La primera celda de la fila tiene el tipo de habitación
    const celdaTipo = fila.querySelector('.room-type-cell');
    return celdaTipo ? celdaTipo.textContent.trim() : 'Desconocido';
}

function verificarYAgregarReserva() {
    const { habitacionNumero, habitacionId, tipoHabitacion, primerFecha, segundaFecha } = seleccionReserva;
    ...
    if (disponible) {
        ...
        // Pintar el rango en naranja
        pintarRangoReserva(habitacionNumero, fechaInicio, fechaFin);
        ...
        console.log("Reserva agregada. Total reservas pendientes:", reservasPendientes.length);
        
        // Resetear selección para permitir otra reserva
        resetearSeleccion();
    ...
    divs.forEach(div => {
        if (parseInt(div.textContent) === numeroHabitacion) {
            ...
            if (fechas.includes(fechaDiv)) {
                div.classList.remove('room-seleccionada', 'room-disponible');
                div.classList.add('room-reservada-pendiente');
                div.style.cursor = 'default';
                div.onclick = null; // Deshabilitar clicks en reservas pendientes
            }
        }
    });
}
...
function siguiente() {
    // Verificar que haya reservas pendientes
    if (reservasPendientes.length === 0) {
        alert('Por favor, seleccione al menos una habitación antes de continuar');
        return;
    }

    // Enviar todas las reservas a la página de confirmación
    const reservasJSON = encodeURIComponent(JSON.stringify(reservasPendientes));
    window.location.href = `/confirmar/reserva?reservas=${reservasJSON}`;
}